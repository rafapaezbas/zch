...
const commands = [
  {
    ...
    command: async (args) => {
      if (args.alias) {
        ...
        const localInvitation = contact.invitation(await contact.contact(masterkey, config.contacts.length))
        config.contacts.push({ alias: args.alias, localInvitation: localInvitation, remoteInvitation: args.invitation, active: true })
        await configuration.saveConfig(config)
        gui.addContact(args.alias, localInvitation)
        if (args.invitation) {
