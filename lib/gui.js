...
const renderChat = (alias, outbox, inbox) => { // localMessages === sent messages, remote Messages === received messages
  resetChat()
  // TODO sort messages by timestamp and render
  inbox.forEach(m => {
    appendChatMessage(new Date(parseInt(m.timestamp.toString())).toISOString(), alias, m.payload)
  })
  outbox.forEach(m => {
    appendChatMessage(new Date(parseInt(m.timestamp)).toISOString(), 'user', m.payload)
  })
}
