...
const { handshake, ack, query } = require('./messages')
...
module.exports = class Relay extends EventEmitter {
  constructor (opts = {}) {
    super()
    this.store = new Corestore(opts.storage || configuration.relayConfigFolder)
    ...
    this.server = this.node.createServer((socket) => {
      ...
      socket.on('data', async (data) => {
        ...
        if (query_ && !entry) { // dont overwrite an address already used
          await this.db.put(query_.address, query_.payload)
  ...
  _error () {
    return c.encode(error, { timestamp: Buffer.from(Data.now().toString()), message: Buffer.from('error') })
  }
}
